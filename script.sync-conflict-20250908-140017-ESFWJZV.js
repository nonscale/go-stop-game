document.addEventListener('DOMContentLoaded', () => {
    ...
    function showChoicePopup(playedCard, choices) {
        ...
        choices.forEach(card => {
            ...
            choiceCardDiv.addEventListener('click', () => {
                ...
                hidePopup();
                finishTurn('player', turnCaptures, null, playedCard.month);
            });
    ...
    function finishTurn(player, turnCaptures, justPlayedOnFloor, playedMonth = null) {
        const mainCaptured = player === 'player' ? playerCaptured : aiCaptured;
        ...
        if (isTadak) {
            showNotificationPopup("따닥!", "따닥! 상대방의 피를 한 장 가져옵니다.");
            stealPi(player);
        }

        if (floor.length === 0 && turnCaptures.length > 0) { // 싹쓸이 check
            showNotificationPopup("싹쓸이!", "바닥을 모두 쓸었습니다! 상대방의 피를 한 장 가져옵니다.");
            stealPi(player);
        }
    ...
    function aiTurn() {
        ...
        // 1. Check for Go/Stop
        const currentScore = calculateScore(aiCaptured, aiGoCount);
        if (currentScore.score >= 3 && !isGoStopTurn) {
            // Simple logic: Go on 3, unless it's the last card or score is high
            if (currentScore.score >= 7 || aiHand.length <= 1) {
                ...
                // The turn will be switched in updateScores after this move
            }
        }

        // 2. Check for special moves (bomb, shake)
        const specialMove = getAISpecialMove();
        if (specialMove.action === 'bomb') {
            showNotificationPopup("폭탄!", "복지장관이 폭탄을 사용합니다!");
            playBomb('ai', specialMove.month);
            return;
        }
        ...
        // 3. Find the best card to play
        const cardToPlay = getAIBestMove();
        if (cardToPlay) {
            playTurn('ai', cardToPlay.id);
        } else {
            if (aiHand.length > 0) {
                playTurn('ai', aiHand[0].id); // Play first card as a fallback
            } else {
                endRound('draw', {});
            }
        }
        ...
        if (!aiShake) {
            for (const month in handCounts) {
                if (handCounts[month] >= 3) {
                    if (handCounts[month] === 3 && floorCounts[month] === 1) {
                        return { action: 'bomb', month: parseInt(month) };
                    }
                    return { action: 'shake' };
                }
            }
    ...
    function updateScores(player) {
        ...
        // AI Go/Stop logic is now handled in aiTurn()
