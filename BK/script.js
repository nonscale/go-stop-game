document.addEventListener('DOMContentLoaded', () => {
    ...
    CARDS.push({ id: 42, month: 11, type: TYPES.PI, name: '11월 (쌍피)', isDoublePi: true, image: 'Hanafuda_November_Tanzaku_Alt.svg.png' });
    ...
    CARDS.push({ id: 46, month: 12, type: TYPES.YEOL, name: '12월 (끗)', image: 'Hanafuda_December_Kasu_1_Alt.svg.png' });
    ...
    let ppukStacks = [], playerShake = false, aiShake = false, bombableMonth = null, canShake = false, playerGoCount = 0, aiGoCount = 0;
    let popupCallback = null;
    let isGoStopTurn = false;

    function loadGameData() {
        playerMoney = parseInt(localStorage.getItem('goStopPlayerMoney') || '1000');
        aiMoney = parseInt(localStorage.getItem('goStopAiMoney') || '1000');
    ...
    function saveMoney() {
        localStorage.setItem('goStopPlayerMoney', playerMoney);
        localStorage.setItem('goStopAiMoney', aiMoney);
    }
    ...
    function showPopup(title, message, onYes, onNo, yesText = '예', noText = '아니오') {
        popupTitle.textContent = title;
        popupMessage.textContent = message;
        popupYesBtn.textContent = yesText;
        popupNoBtn.textContent = noText;
        popupYesBtn.style.display = onYes ? 'inline-block' : 'none';
        popupNoBtn.style.display = onNo ? 'inline-block' : 'none';
        popupChoicesDiv.style.display = 'none';
        popupOverlay.classList.remove('hidden');
        popupCallback = { onYes, onNo };
    }

    function showNotificationPopup(title, message) {
        showPopup(title, message, hidePopup, null, '확인');
    }
    ...
    function showChoicePopup(playedCard, choices) {
        ...
        choices.forEach(card => {
            ...
            choiceCardDiv.addEventListener('click', () => {
                const chosenCard = choices.find(c => c.id === card.id);
                floor.splice(floor.findIndex(c => c.id === chosenCard.id), 1);
                const turnCaptures = [playedCard, chosenCard];
                hidePopup();
                finishTurn('player', turnCaptures, null);
            });
            popupChoicesDiv.appendChild(choiceCardDiv);
        });
        popupYesBtn.style.display = 'none';
        popupNoBtn.style.display = 'none';
    ...
    function startGame() {
        ...
        bombableMonth = null; canShake = false; isGoStopTurn = false;
    ...
    function endRound(winner, scoreDetails) {
        if (winner === 'draw') {
            showNotificationPopup("무승부", "이번 판은 무승부입니다!");
        ...
        let message = `${winner === 'player' ? '김여사' : '복지장관'}님이 ${score}점으로 승리!`;
        ...
        showNotificationPopup("라운드 종료", message + `\n\n총 획득 금액: ${finalWinnings}원`);
        ...
        if (aiMoney <= 0) {
            showNotificationPopup("게임 종료", `복지장관이 파산했습니다! 김여사님의 최종 승리! 새 게임을 시작합니다.`);
    ...
    function playTurn(player, cardId) {
        ...
        if (ppukIndex > -1) {
            showNotificationPopup("뻑!", `${playedCard.month}월 뻑을 해결했습니다!`);
        ...
        if (matches.length === 0) {
            ...
            finishTurn(player, [], justPlayedOnFloor);
            ...
            if (player === 'player') {
                showChoicePopup(playedCard, matches);
            } else {
                const bestCard = chooseBestCard(matches);
                floor.splice(floor.findIndex(c => c.id === bestCard.id), 1);
                turnCaptures.push(playedCard, bestCard);
                finishTurn(player, turnCaptures, null);
            }
        }
    }

    function playBomb(player, month) {
        const hand = player === 'player' ? playerHand : aiHand;
        const mainCaptured = player === 'player' ? playerCaptured : aiCaptured;
        if (player === 'player') playerShake = true; else aiShake = true;
        const bombCardsInHand = hand.filter(c => c.month === month);
        const bombCardOnFloor = floor.find(c => c.month === month);
        if (player === 'player') playerHand = hand.filter(c => c.month !== month); else aiHand = hand.filter(c => c.month !== month);
        if(bombCardOnFloor) floor = floor.filter(c => c.month !== month);
        mainCaptured.push(...bombCardsInHand);
        if(bombCardOnFloor) mainCaptured.push(bombCardOnFloor);
        stealPi(player);
        finishTurn(player, [], null);
    }

    function finishTurn(player, turnCaptures, justPlayedOnFloor) {
        const mainCaptured = player === 'player' ? playerCaptured : aiCaptured;
        const drawnCard = deck.pop();
        if (!drawnCard) { endRound('draw', {}); return; }
        if (justPlayedOnFloor && drawnCard.month === justPlayedOnFloor.month) {
            showNotificationPopup('쪽!', '쪽! 축하합니다!');
            stealPi(player);
            const idx = floor.findIndex(c => c.id === justPlayedOnFloor.id);
            if (idx > -1) turnCaptures.push(floor.splice(idx, 1)[0]);
        }
        ...
        if (matches.length === 0) {
            ...
            const bestCard = chooseBestCard(matches);
            if (player === 'player') {
                showNotificationPopup("자동 선택", `바닥에 같은 월의 카드가 여러 장 있어, 가장 좋은 패(${bestCard.name})를 자동으로 획득합니다.`);
            }
            floor.splice(floor.findIndex(c => c.id === bestCard.id), 1);
        ...
        for (const m in counts) {
            if (counts[m] === 3 && !ppukStacks.includes(parseInt(m))) {
                showNotificationPopup("뻑!", `뻑! - ${m}월 카드가 바닥에 3장이 되었습니다!`);
                ppukStacks.push(parseInt(m));
            }
        }
        render();
        const goStopTriggered = updateScores(player);
        if (!goStopTriggered) {
            switchTurn(player);
        }
    }
    ...
    function handleSpecialActions() {
        ...
        checkForSpecials();
        if (bombableMonth) {
            showPopup('폭탄!', `${bombableMonth}월 폭탄을 사용하시겠습니까?`, 
                () => { hidePopup(); playBomb('player', bombableMonth); },
                () => { const wasBomb = bombableMonth; hidePopup(); if (wasBomb) { bombableMonth = null; handleSpecialActions(); } }
            );
        } else if (canShake) {
            showPopup('흔들기!', '패에 같은 월의 카드가 3장 있습니다. 흔드시겠습니까?', 
                () => { hidePopup(); playerShake = true; showNotificationPopup("흔들기", "김여사님이 흔들었습니다! 이번 판은 점수가 2배가 됩니다."); },
                () => { hidePopup(); }
            );
        }
    }

    function handleGoStopPopup() {
        const { score } = calculateScore(playerCaptured, playerGoCount);
        showPopup('고 또는 스톱', `현재 점수는 ${score}점 입니다. '고' 하시겠습니까?`,
            () => { 
                playerGoCount++;
                showNotificationPopup("고!", '김여사님이 고!를 외쳤습니다.');
                hidePopup(); 
                isGoStopTurn = false; 
                switchTurn('player'); 
            },
            () => { 
                hidePopup(); 
                isGoStopTurn = false; 
                const scoreDetails = calculateScore(playerCaptured, playerGoCount, true);
                endRound('player', scoreDetails);
            },
            '고', '스톱'
        );
    }
    ...
    function aiTurn() {
        ...
        if (decision.action === 'bomb') {
            showNotificationPopup("폭탄!", "복지장관이 폭탄을 사용합니다!");
            playBomb('ai', decision.month);
            return;
        }
        if (decision.action === 'shake') {
            aiShake = true;
            showNotificationPopup("흔들기!", "복지장관이 흔들었습니다! 이번 판은 점수 2배!");
        }
        const isLosing = playerMoney < 500;
        const isWinning = getWeeklyWinnings().amount > 7000;
        let cardToPlay = isLosing ? findWorstMove() : (isWinning ? findBestMove() : null);
        if (!cardToPlay) {
            if (aiHand.length === 0) { endRound('draw', {}); return; }
            cardToPlay = aiHand[Math.floor(Math.random() * aiHand.length)];
        }
        playTurn('ai', cardToPlay.id);
    }

    function getAIDecision() {
        const handCounts = aiHand.reduce((acc, c) => { acc[c.month] = (acc[c.month] || 0) + 1; return acc; }, {});
        const floorCounts = floor.reduce((acc, c) => { acc[c.month] = (acc[c.month] || 0) + 1; return acc; }, {});
        if (!aiShake) {
            for (const month in handCounts) {
                if (handCounts[month] >= 3) {
                    if (floorCounts[month] === 1) return { action: 'bomb', month: parseInt(month) };
                    return { action: 'shake' };
                }
            }
        }
    ...
    function findBestMove() {
        const priorities = [TYPES.GWANG, TYPES.YEOL, TYPES.TTI];
        for (const type of priorities) {
            for (const myCard of aiHand) {
                const matchOnFloor = floor.find(floorCard => floorCard.month === myCard.month && floorCard.type === type);
                if (matchOnFloor) return myCard;
            }
        }
        return null;
    }
    ...
    function findWorstMove() {
        const reversedPriorities = [TYPES.GWANG, TYPES.YEOL, TYPES.TTI];
        for (const type of reversedPriorities) {
            const worstCard = aiHand.find(aiCard => {
                const hasMatch = floor.some(floorCard => floorCard.month === aiCard.month);
                return aiCard.type === type && !hasMatch;
            });
            if (worstCard) return worstCard;
        }
        return null;
    }
    ...
    function updateScores(player) {
        ...
        if (player === 'player') {
            // Show Go/Stop popup only if the player scored 7+ AND has cards left to play
            if (score >= 7 && !isGoStopTurn && playerHand.length > 0) {
                isGoStopTurn = true;
                handleGoStopPopup();
                return true; // Go/Stop decision initiated
            }
        } else { // AI's turn
            if (score >= 3 && !isGoStopTurn) {
                if (score >= 7 || aiHand.length === 0) { // AI stops at 7+ or if it's their last card
                    const scoreDetails = calculateScore(aiCaptured, aiGoCount, true);
                    endRound('ai', scoreDetails);
                } else { // AI goes
                    aiGoCount++;
                    showNotificationPopup("고!", '복지장관이 고!를 외쳤습니다.');
                    switchTurn('ai');
                }
        ...
        if (deck.length === 0 && playerHand.length === 0 && aiHand.length === 0) {
            const playerScore = calculateScore(playerCaptured, playerGoCount, true);
            const aiScore = calculateScore(aiCaptured, aiGoCount, true);
            if (playerScore.score > aiScore.score) {
                endRound('player', playerScore);
    ...
    function calculateScore(captured, goCount = 0, isFinal = false) {
        let score = 0;
        ...
        const winner = currentPlayer === 'player' ? 'player' : 'ai';
        ...
        const shakeBonus = (winner === 'player' && playerShake) || (winner === 'ai' && aiShake);
    ...
    function stealPi(player) {
        ...
        if (piIndex > -1) {
            ...
            showNotificationPopup("피 뺏기", `상대방의 피를 한 장 가져옵니다!`);
        }
    }
    ...
    popupYesBtn.addEventListener('click', () => {
        if (popupCallback && typeof popupCallback.onYes === 'function') {
            popupCallback.onYes();
        }
    });

    popupNoBtn.addEventListener('click', () => {
        if (popupCallback && typeof popupCallback.onNo === 'function') {
            popupCallback.onNo();
        } else {
            hidePopup();
        }
    ...
    function checkForSpecials() {
        ...
        for (const month in handCounts) {
            if (handCounts[month] >= 3) {
                canShake = true;
                const floorMatch = floor.find(c => c.month === parseInt(month));
                if (handCounts[month] === 3 && floorMatch) {
                    bombableMonth = parseInt(month);
                }
            }
        }
    }
